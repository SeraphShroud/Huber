...
// Paul Epperson
public class LoginToBarberTest extends ActivityInstrumentationTestCase2<LoginSignupActivity> {
    ...
    public void testZLoginButton() {
        ...
        // !! AND AN EXISTING BARBER USERNAME, PASSWORD IS ENTERED
        testPreconditions();

        // Set up Activity Monitor
        Instrumentation.ActivityMonitor yourBarberWelcomeMonitor
                = mInstrumentation.addMonitor(BarberWelcome.class.getName(),
                null, false);

        // Set up known existing barber input username, password
        mLoginSignupActivity.runOnUiThread(new Runnable() {
            public void run() {
                mUsernameEditText.setText("cse101");
        ...
        // !! THEN BARBER WELCOME PAGE LOADS
        // Wait for BarberWelcome activity to load
        BarberWelcome receiverActivity = (BarberWelcome)yourBarberWelcomeMonitor.
                waitForActivityWithTimeout(TIMEOUT_IN_MS);

        // Check BarberWelcome has loaded
        assertNotNull(receiverActivity);
        assertEquals(1, yourBarberWelcomeMonitor.getHits());

        // Remove Monitor
        getInstrumentation().removeMonitor(yourBarberWelcomeMonitor);
